...
aci_status_code_t Notif::bond_data_restore( uint8_t eeprom_status, bool *bonded_first_time_state)
{
    ...
    while(1)
    {
        ...
        {
            debug_println(F("bond_data_restore: Cmd Q Full"));
        ...
        while (1)
        {
            if (lib_aci_event_get(&aci_state, &aci_data))
            {
                ...
                if (ACI_EVT_CMD_RSP != aci_evt->evt_opcode)
                {
                    ...
                    debug_print(F("bond_data_restore: Expected cmd rsp evt. Got: 0x"));
            ...
            else
            {
                //Manage the bond in EEPROM of the AVR
                {
                    ...
                    {/*
                        ...
                        {
                            debug_println(F("Bond information loaded into nRF8001"));
                        }
                        else
                        {
...
void Notif::CommandResponse( aci_evt_t *aci_evt) {
    ...
    switch (aci_evt->params.cmd_rsp.cmd_opcode) {
        ...
        default:
            debug_print(F("Evt Unk Cmd: "));
            debug_println(  aci_evt->params.cmd_rsp.cmd_opcode); //hex(aci_evt->params.cmd_rsp.cmd_opcode);
    }
}
...
void Notif::HwError(aci_evt_t *aci_evt)
{
    ...
    {
        debug_print(aci_evt->params.hw_error.file_name[counter]); //uint8_t file_name[20];
    }
    debug_println();
#endif
    //Manage the bond in EEPROM of the AVR
