...
void ancs_get_notification_data(uint32_t uid) {
    if (command_send_enable || ((millis() - last_command_send) > 1000)) {
    debug_print(F("[ANCS NS] ancs_get_notification_data("));
    debug_print(uid, DEC);
    debug_println(F(")"));
    ...
    debug_println(command_send_enable);
         ...
         debug_print(F("[ANCS NS] ancs_get_notification_data("));
         debug_print(uid, DEC);
         debug_print(F(") - Need Command Send - "));
         debug_println(millis() - last_command_send);
...
void ancs_notification_source_parser(const uint8_t* buffer) {
    debug_println(F("[ANCS NS] ancs_notification_source_parser()"));
    uint8_t event_id;
    uint8_t event_flags;
    uint8_t category_id;
...
#endif
        default:
            debug_println(F("UNKOWN CATEGORY"));
            return;
    }
    debug_print(F("["));
